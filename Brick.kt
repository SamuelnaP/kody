...
class Brick(context : Context, val width : Int, val height : Int, val xx:Int,val yy:Int) {
    var x = xx
    var y = yy
    var Xsize = 100
    var Ysize = 50
    ...
    companion object { // staaticky blok
        var ballCounter = 0
        lateinit var bitmapbb : Bitmap
    }
    init {

        bitmapbb = BitmapFactory.decodeResource(context.resources, R.drawable.brick)
    }
    fun update() {
    }
    ...
    fun draw(canvas: Canvas) {
        ...
        canvas.drawBitmap(bitmapbb, null, Rect(x,y,(x+Xsize), (y+Ysize)), mPaint)
